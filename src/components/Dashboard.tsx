...
function Shell({ children, profile, session }: { children: ReactNode; profile: Profile; session: Session }) {
  ...
  return <main className="workspace"><header className="workspace-nav"><Link className="logo" to="/">arco<span>nnet</span></Link><div><span className="role-label">{profile.role} workspace</span><button onClick={signout}>Sign out</button></div></header>{children}</main>;
}
...
function AdminDashboard({ profile, session }: { profile: Profile; session: Session }) {
  ...
  return <Shell profile={profile} session={session}><section className="workspace-hero"><p className="mono">[ ADMIN ]</p><h1>Keep the<br /><em>standard clear.</em></h1><p>Review every architect application before it becomes visible in the Arconnet directory.</p></section><section className="admin-list"><div className="workspace-title"><h2>Review queue</h2><span>{applications.length} waiting</span></div>{error && <p className="form-error">{error}</p>}{applications.map(app => <article key={app.id}><div><p className="mono">{app.city || 'Location not added'} · {app.specialty || 'Speciality not added'}</p><h3>{app.studio_name}</h3><p>{app.bio || 'No practice statement yet.'}</p></div><div className="admin-actions"><button onClick={() => update(app.id, 'changes_requested')}>Request changes</button><button className="approve" onClick={() => update(app.id, 'approved')}>Approve</button></div></article>)}{!applications.length && !error && <Empty title="Your review queue is clear." text="New architect submissions will appear here." />}</section></Shell>;
}

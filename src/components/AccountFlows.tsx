  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (mode === 'signup') {
        ...
        formEl.reset();
        setMode('signin');
        setToast('Account created — welcome to Arconnet. Please sign in to continue.');
      } else {
...
export function ArchitectApply() {
  ...
  return <main className="account-flow application"><Link className="logo" to="/">arco<span>nnet</span></Link><section><p className="mono">[ ARCHITECT APPLICATION · 01 / 01 ]</p><h1>{locked ? <>Already<br /><em>submitted.</em></> : <>Let’s meet<br /><em>your practice.</em></>}</h1>
    {locked ? <>
      <p className="flow-intro">{status === 'approved'
        ? 'Your application was approved — your profile is live in the Arconnet directory and is no longer editable from this page.'
        : 'Your application has been submitted and is under review. It cannot be edited while it is with the Arconnet team.'}</p>
